...
export function synthesizePlan(e = {}) {
  ...
  const data = pct([!!e.runs_weekly_scorecard, hasList(e.scorecard_kpis)])
  const process = pct([hasList(e.core_processes), has(e.sop_maturity)])
  const issues = pct([hasList(e.issues), has(e.pain_points)])
  const traction = pct([!!e.runs_l10, has(e.weekly_meeting_day), hasList(e.quarterly_rocks)])
  ...
  if (!hasList(e.core_processes)) findings.push('Core processes not documented.')
  ...
  if (!hasList(e.issues)) findings.push('Issues list not captured.')
  ...
  if (!hasList(e.core_processes)) rocks.push('Document the 3–7 core processes.')
