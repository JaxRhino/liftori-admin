...
// jsonb-array <-> newline text helpers
...
export default function FeatureLibrary() {
  ...
  function openNew() {
    setIsNew(true); setSelKey('__new__'); setDraft({ ...BLANK })
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-white/10">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-white/50 mt-1 max-w-2xl">The deep buildout spec for every Liftori feature. Improve them over time, then drop them straight into a build &mdash; the picker reads the same catalog.</p>
          </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-[340px_1fr] min-h-0">
        {/* List */}
        <div className="border-r border-white/10 overflow-y-auto p-4 space-y-5">
          {loading && <div className="text-sm text-white/40 px-2">Loading catalog…</div>}
          {!loading && filtered.length === 0 && <div className="text-sm text-white/40 px-2">No features match.</div>}
          {grouped.map(([cat, items]) => (
            <div key={cat}>
              <div className="text-[11px] font-semibold uppercase tracking-wider text-white/40 px-2 mb-1.5">{cat} <span className="text-white/25">({items.length})</span></div>
              <div className="space-y-1">
                {items.map(r => (
                  <button key={r.key} onClick={() => openFeature(r)} className={`w-full text-left rounded-lg border px-3 py-2.5 transition-colors ${selKey === r.key ? 'border-brand-blue/50 bg-brand-blue/10' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'} ${r.active === false ? 'opacity-50' : ''}`}>
        ...
        <div className="overflow-y-auto">
          {!draft && (
            <div className="h-full flex flex-col items-center justify-center text-center px-6 py-24">
              <div className="text-white/30 text-sm max-w-sm">Select a feature to view and edit its full buildout spec, or create a new one.</div>
            </div>
          )}
          {draft && (
            <div className="max-w-3xl mx-auto p-6 space-y-6">
              {/* Sticky action bar */}
              <div className="flex items-center justify-between gap-3 sticky top-0 -mt-6 pt-6 pb-3 bg-navy-900/95 backdrop-blur z-10 border-b border-white/10">
              ...
              <section className="space-y-4">
                ...
                <Field label="Problem it solves"><Area value={draft.problem} onChange={e => set('problem', e.target.value)} rows={2} /></Field>
                <Field label="Value / outcome"><Area value={draft.value} onChange={e => set('value', e.target.value)} rows={2} /></Field>
              </section>
