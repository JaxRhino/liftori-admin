              ...
              <div className="space-y-2">
                {PLATFORMS.map(platform => (
                  <button key={platform.id} onClick={() => togglePlatform(platform.id)}
                    ...
                    <div className={selectedPlatforms.includes(platform.id) ? platform.textColor : 'text-slate-500'}>{platform.icon}</div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium">{platform.label}</div>
                      <div className="text-xs opacity-60">{platform.limit.toLocaleString()} char limit</div>
                    </div>
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                      selectedPlatforms.includes(platform.id) ? 'border-current bg-current' : 'border-slate-600'
                    }`}>
                      {selectedPlatforms.includes(platform.id) && (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs text-slate-500 bg-slate-700/50 px-2 py-0.5 rounded-full">{post.contentType}</span>
                      </div>
