...
import { WorkspaceTabBody, Narrative, WORKSPACE_TAB_KEYS, PRODUCT_TYPES } from '../components/BuildWorkspace'
...
export default function ProjectDetail() {
  ...
  async function saveWs(next) {
    ...
    setWsSaving(true)
    try {
      const { error } = await supabase.from('projects').update({ workspace: next, updated_at: new Date().toISOString() }).eq('id', id)
      if (error) throw error
    } catch (err) { console.error('Error saving workspace:', err) } finally { setWsSaving(false) }
  }

  async function updateProjectType(newType) {
    const { error } = await supabase.from('projects').update({ project_type: newType }).eq('id', id)
    if (!error) setProject(p => ({ ...p, project_type: newType }))
  }
    ...
    <div className="p-8">
      ...
      {topTab === 'devlab' ? (
        ...
        <>
          ...
          {/* ===== SHARED WORKSPACE SPEC TABS (uniform with In-House Builds) ===== */}
          {WORKSPACE_TAB_KEYS.includes(activeTab) && (
            <WorkspaceTabBody tab={activeTab} ws={ws} onSave={saveWs} productType={{ value: project.project_type, options: PRODUCT_TYPES, onChange: updateProjectType }} />
          )}
          {activeTab === 'notes' && (
            <Narrative title="Notes" value={ws.notes} onSave={v => saveWs({ ...ws, notes: v })} placeholder="Project notes, decisions, context..." />
          )}
