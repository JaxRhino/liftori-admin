...
import { WorkspaceTabBody, wsTabBadge, Stat, money, PRODUCT_TYPES, WORKSPACE_TAB_KEYS } from '../components/BuildWorkspace'
...
const MS_STATUS_ICONS = { pending: 'o', in_progress: '*', completed: '#', blocked: 'x' }
...
export default function InHouseBuildDetail() {
  ...
  const tabBadge = (key) => {
    ...
    return wsTabBadge(ws, key)
  }

  return (
    <div className="p-6">
      <button onClick={() => navigate('/admin/builds')} className="text-slate-400 hover:text-white text-sm mb-4 flex items-center gap-1 transition-colors">&larr; Back to Builds</button>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          {(() => {
            ...
            const show = d.owner || d.assigned_to || mrr || arr || tb || d.stage || d.product_type
            if (!show) return null
            return (
              <div className="bg-navy-800 border border-navy-700/50 rounded-lg p-5">
                ...
                {(d.assigned_to || d.stage || d.pricing_model || d.product_type) && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {d.product_type && <span className="text-xs px-2.5 py-1 bg-navy-900 text-slate-300 rounded-lg border border-navy-700/50">{d.product_type}</span>}
      ...
      {/* Shared workspace-backed spec tabs */}
      {WORKSPACE_TAB_KEYS.includes(activeTab) && (
        <WorkspaceTabBody tab={activeTab} ws={ws} onSave={saveWs} productType={productType} />
      )}
