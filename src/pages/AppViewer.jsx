...
/**
 * App Viewer (/admin/products/:slug)
 *
 * Generalized, multi-app version of MobilePreview. Click any app under the
 * "Mobile Apps" nav section and land here: a phone-framed live preview iframe
 * on the left, an ops + download panel on the right.
 *
 * Each app is described in the APPS registry below. An app with a `repo`
 * + `eas` block is "live" (full preview/build/download). An app without one
 * is "planned" — we still render the page, but with an honest empty state
 * instead of fake build config.
 *
 * Per-app preview URLs are read from Vite env vars (configurable in Vercel)
 * with an ad-hoc URL bar as a fallback so anyone on shift can paste a
 * `npx expo start --web` URL without redeploying.
 */
...
const APPS = {
  'liftori-app': {
    slug: 'liftori-app',
    name: 'Liftori App',
    tagline: 'Liftori mobile companion — Expo + React Native',
    status: 'live',
    // preview URL resolution order: app-specific env → legacy env → ad-hoc
    previewUrl:
...
export default function AppViewer() {
  ...
  if (!app) return <NotFound slug={slug} />

  return <AppView key={app.slug} app={app} />
}

function AppView({ app }) {
  const [url, setUrl] = useState(app.previewUrl)
  const [deviceKey, setDeviceKey] = useState('iphone14')
  const [orientation, setOrientation] = useState('portrait')
  const [iframeKey, setIframeKey] = useState(0)
  const [iframeLoaded, setIframeLoaded] = useState(false)
  const [urlInput, setUrlInput] = useState(app.previewUrl)
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">{app.tagline}</p>
        </div>
        ...
        <aside className="space-y-4">
          ...
          <OpsCard icon={<Download className="h-4 w-4 text-emerald-400" />} title="Download">
            {repo ? (
              ...
              <p className="text-xs leading-relaxed text-gray-400">
                No downloadable build yet — this app hasn't been provisioned with a repo or EAS project.
              </p>
            )}
          </OpsCard>
          ...
          {app.status === 'planned' && (
            <OpsCard icon={<Rocket className="h-4 w-4 text-amber-400" />} title="Status — Planned">
              <p className="text-xs leading-relaxed text-gray-300">{app.plannedNote}</p>
              {(app.links || []).map((l) => (
                l.internal ? (
                  <NavLink
                    key={l.to}
                    to={l.to}
                    className="mt-2 flex items-center justify-between gap-3 rounded-lg border border-white/5 bg-navy-800/40 px-3 py-2 text-sm text-gray-200 transition-colors hover:bg-navy-700/60 hover:text-white"
                  >
                    <span className="flex items-center gap-2">
                ...
                {app.channels.map((c) => (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${c.tint}`}>
                          {c.label}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-gray-400">{c.description}</p>
                    </div>
                    <CopyChip value={c.key} label={`channel-${c.key}`} copy={copy} copied={copied} />
                  </div>
                ))}
              </div>
            </OpsCard>
          ...
          {repo && (
            <OpsCard icon={<Package className="h-4 w-4 text-purple-400" />} title="Deep links">
          ...
          {eas && (
            <OpsCard icon={<Info className="h-4 w-4 text-sky-400" />} title="Shipping rules">
              <p className="text-xs leading-relaxed text-gray-300">
                ...
                fires <span className="font-mono">eas-update</span> — installed APKs grab the new JS bundle in ~30s and apply it on next cold start.
              </p>
              <p className="text-xs leading-relaxed text-gray-300">
                <span className="font-semibold text-white">Native rebuild</span>{' '}
                (~15 min) is manual-only. Required when bumping <span className="font-mono">version</span>, adding a permission, or installing a native plugin. Skip OTAs with <span className="font-mono">[skip update]</span> in the commit.
              </p>
            </OpsCard>
          )}
        </aside>
...
function PhoneFrame({ width, height, scale, hasNotch, isDesktop, children }) {
  ...
  return (
    <div style={{ width: scaledWidth + bezel * 2, height: scaledHeight + bezel * 2 }} className="relative">
      ...
      >
        <div
          style={{ width, height, borderRadius: isDesktop ? 8 : 32 }}
...
function EmptyPreview({ app }) {
  return (
    <div className="mx-auto max-w-md rounded-xl border border-dashed border-white/10 bg-navy-800/40 p-8 text-center">
      <AlertTriangle className="mx-auto h-8 w-8 text-amber-400" />
      <h3 className="mt-3 text-base font-semibold text-white">No preview URL configured</h3>
      {app.status === 'planned' ? (
        <p className="mt-2 text-sm text-gray-400">
          {app.name} doesn't have a live preview yet. Paste a dev URL above once the Expo project is running.
        </p>
      ) : (
        <p className="mt-2 text-sm text-gray-400">
          Paste a URL into the bar above, or set{' '}
          <code className="rounded bg-navy-900 px-1.5 py-0.5 text-xs text-brand-blue">VITE_LIFTORI_APP_PREVIEW_URL</code>{' '}
      ...
      <div className="mt-4 rounded-lg bg-navy-900 p-3 text-left">
        ...
        <pre className="mt-1 whitespace-pre-wrap break-all font-mono text-xs text-emerald-300">
          {app.repo ? `cd ${app.repo.name} && npx expo start --web` : 'npx expo start --web'}
        </pre>
        <p className="mt-2 text-[11px] text-gray-400">
          Then load <span className="font-mono text-white">http://localhost:8081</span> above.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="p-6">
      <div className="mx-auto max-w-lg rounded-xl border border-dashed border-white/10 bg-navy-800/40 p-8 text-center">
        <AlertTriangle className="mx-auto h-8 w-8 text-amber-400" />
        <h1 className="mt-3 text-xl font-bold text-white">App not found</h1>
        <p className="mt-2 text-sm text-gray-400">
          No app is registered under <span className="font-mono text-brand-blue">{slug}</span>. Pick one below:
        </p>
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          {Object.values(APPS).map((a) => (
            <NavLink
              key={a.slug}
              to={`/admin/products/${a.slug}`}
              className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-navy-800/70 px-3 py-1.5 text-sm text-gray-200 hover:bg-navy-700"
            >
              <Smartphone className="h-3.5 w-3.5" />
              {a.name}
            </NavLink>
