...
const PRIORITIES = [
  ...
  { value: 'low',    label: 'Low',    color: 'text-gray-400',  bg: 'bg-gray-500/10 border-gray-500/20' },
]
const STATUSES = [
...
const TEXTAREA = 'w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm placeholder-gray-500 focus:outline-none focus:border-blue-500 resize-y'
    ...
    <div className="p-6 max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Track and manage your work items</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={s.label} className="bg-navy-900 border border-white/10 rounded-xl p-4">
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm placeholder-gray-500 focus:outline-none focus:border-blue-500 w-56" />
        <div className="flex gap-1 bg-navy-800/50 rounded-lg p-1">
          {[{ value: 'all', label: 'All' }, ...STATUSES.map(s => ({ value: s.value, label: s.label }))].map(s => (
            <button key={s.value} onClick={() => setFilterStatus(s.value)}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${filterStatus === s.value ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}>
              {s.label}
            </button>
          ))}
        </div>
        <div className="flex gap-1 bg-navy-800/50 rounded-lg p-1">
          {[{ value: 'all', label: 'All Priority' }, ...PRIORITIES.map(p => ({ value: p.value, label: p.label }))].map(p => (
            <button key={p.value} onClick={() => setFilterPriority(p.value)}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${filterPriority === p.value ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}>
              {p.label}
            </button>
          ))}
        </div>
        <span className="text-gray-500 text-xs ml-auto">{filtered.length} task{filtered.length !== 1 ? 's' : ''}</span>
      </div>

      {/* Form */}
      {showForm && (
        <div className="bg-navy-900 border border-blue-500/30 rounded-xl p-5 mb-5 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { setShowForm(false); setEditing(null) }}
              className="text-gray-400 hover:text-white px-4 py-2 text-sm transition-colors">Cancel</button>
          </div>
        </div>
      )}
          ...
          {STATUSES.map(col => (
            <div key={col.value}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-gray-500 ml-auto">{grouped[col.value].length}</span>
              </div>
              <div className="space-y-2">
                {grouped[col.value].map(task => {
                  ...
                  return (
                    <div key={task.id}
                      className={`bg-navy-900 border rounded-xl p-3.5 group ${overdue ? 'border-amber-500/30' : 'border-white/10 hover:border-white/20'} transition-colors`}>
                      <div className="flex items-start gap-2">
                        <button onClick={() => cycleStatus(task)}
                          className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-colors ${
                            task.status === 'done' ? 'bg-emerald-500 border-emerald-500' :
                            task.status === 'in_progress' ? 'border-blue-500' : 'border-navy-600 hover:border-gray-400'
                          }`}>
                          {task.status === 'done' && (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-2 flex-wrap">
                            ...
                            {task.due_date && (
                              <span className={`text-xs ${overdue ? 'text-amber-400' : 'text-gray-500'}`}>
                      ...
                      <div className="flex gap-2 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEdit(task)}
                          className="text-xs text-gray-400 hover:text-white px-2 py-1 rounded border border-white/10 transition-colors">Edit</button>
                ...
                {grouped[col.value].length === 0 && (
                  <div className="border border-dashed border-white/10 rounded-xl p-4 text-center text-navy-600 text-xs">
