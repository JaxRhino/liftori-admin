    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 py-4 space-y-1 px-2 overflow-y-auto" onClick={(e) => { if (e.target.closest('a')) { setMobileMenuOpen(false); setSidebarHovered(false); } }}>
          ...
          {/* Liftori Products -- unified hub. Each CRM + its app = one project at /admin/products. */}
          {!isImpersonating && <div className="pt-1">
            {sidebarOpen && (
              <p className="text-xs text-slate-600 uppercase tracking-widest px-3 mb-1 mt-2">Liftori Products</p>
            )}
            <NavLink
              to="/admin/products"
              className={({ isActive }) =>
              ...
              {sidebarOpen && <span className="flex-1 text-left">Products</span>}
            </NavLink>
          </div>}
          </>)}
