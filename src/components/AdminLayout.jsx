...
const PORTAL_INDUSTRIES = [
  ...
  { industry: 'KEC', label: 'CSC Services', to: '/crm/88888888-0002-0000-0000-000000000001/dashboard', live: true },
...
const opsItems = [
  ...
  {
    label: 'New Tenant', path: '/admin/new-tenant', icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    label: 'Platform Fees', path: '/admin/platform-fees', icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const marketingHubItems = [
  // Wave G1 - sidebar prune (Wave G — Marketing Hub roadmap).
  // From 14 surfaces -> 7 Core. The 11 hidden are: Marketing Tracker, Ad Manager,
  // On Pace Tracking, Content Creator, Scheduler, Customer Map, SEO Manager,
  // Analytics, Social Listening, UTM Builder, A/B Testing. Routes stay live
  // (deep links don't break); just hidden from nav until their backends ship.
  // Order = funnel: overview -> create -> send -> segment -> capture -> measure.
  {
  ...
  {
    label: 'Social Composer', path: '/admin/marketing/social-composer', icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    label: 'Email Sequences', path: '/admin/marketing/sequences', icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  {
    label: 'Waitlist', path: '/admin/marketing/waitlist', icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
      </svg>
    )
  ...
  {
    label: 'Performance', path: '/admin/marketing/performance', icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const navItems = [
  ...
  {
    label: 'Liftori Settings', path: '/admin/liftori-settings', icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function AdminLayout() {
  ...
  const [crmsOpen, setCrmsOpen] = useState(true)
  const [mobileAppsOpen, setMobileAppsOpen] = useState(true)
  ...
  const isToolsRoute = ['/admin/tasks', '/admin/notes', '/admin/calendar', '/admin/rally', '/admin/company-docs', '/admin/settings'].some(p => location.pathname.startsWith(p))
        ...
        <nav className="flex-1 py-4 space-y-1 px-2 overflow-y-auto" onClick={(e) => { if (e.target.closest('a')) { setMobileMenuOpen(false); setSidebarHovered(false); } }}>
              ...
              {['Call Center', 'Marketing', 'Finance', 'Communications', 'EOS'].includes(item.label) ? (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                    ${(item.label === 'Call Center' && isCallCenterRoute) || (item.label === 'Marketing' && isMarketingRoute) || (item.label === 'Finance' && isFinanceRoute) || (item.label === 'Communications' && isCommsRoute) || (item.label === 'EOS' && isEOSRoute)
                  ...
                  {sidebarOpen && (
                    <>
                      ...
                      <svg className={`w-4 h-4 transition-transform ${
                        (item.label === 'Call Center' && callCenterOpen) || (item.label === 'Marketing' && marketingOpen) || (item.label === 'Finance' && financeOpen) || (item.label === 'Communications' && commsOpen) || (item.label === 'EOS' && eosOpen) ? 'rotate-180' : ''
                ...
                {showOps && showOpsHub && <div>
                  ...
                  {sidebarOpen && opsOpen && (
                    <div className="ml-3 pl-3 border-l border-white/10 mt-1 space-y-0.5">
                      ...
                      {/* Liftori Team -- nested dropdown */}
                      <button onClick={() => setLiftoriTeamOpen(o => !o)} className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-xs font-medium text-gray-400 hover:text-white hover:bg-navy-700/50 transition-colors">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" /></svg>
                        <span className="flex-1 text-left">Liftori Team</span>
                        <svg className={`w-3.5 h-3.5 transition-transform ${liftoriTeamOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                      </button>
                      {liftoriTeamOpen && (
                        <div className="ml-3 pl-3 border-l border-white/10 space-y-0.5">
          ...
          {showBuilds && !isImpersonating && <div className="pt-1">
            ...
            <NavLink
              ...
              {sidebarOpen && <span className="flex-1 text-left">Custom Builds</span>}
            </NavLink>
            <NavLink
          ...
          {/* Liftori Products -- admin only. CRMS (industry portals) + Mobile Apps. */}
          {!isImpersonating && <div className="pt-1">
            {sidebarOpen && (
              <button onClick={() => setLiftoriProductsOpen(o => !o)} className="w-full flex items-center justify-between px-3 mb-1 mt-2 text-xs text-slate-600 uppercase tracking-widest hover:text-slate-300 transition-colors">
                <span>Liftori Products</span>
                <svg className={`w-3 h-3 transition-transform ${liftoriProductsOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
              </button>
            )}
            {(!sidebarOpen || liftoriProductsOpen) && (
              <div>
                {sidebarOpen && (
                  <button onClick={() => setCrmsOpen(o => !o)} className="w-full flex items-center justify-between px-3 mt-1 mb-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-500 hover:text-slate-300 transition-colors">
                    <span>CRMS</span>
                ...
                {(!sidebarOpen || crmsOpen) && PORTAL_INDUSTRIES.map((g) => (
                ...
                {sidebarOpen && (
                  <button onClick={() => setMobileAppsOpen(o => !o)} className="w-full flex items-center justify-between px-3 mt-2 mb-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-500 hover:text-slate-300 transition-colors">
                ...
                {(!sidebarOpen || mobileAppsOpen) && [{ label: 'BOLO Go', to: '/admin/products/bolo-go' }, { label: 'Liftori App', to: '/admin/products/liftori-app' }].map((m) => (
                  <NavLink
                    key={m.to}
                    to={m.to}
                    className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                      isActive ? 'text-white bg-navy-700/70' : 'text-gray-300 hover:text-white hover:bg-navy-700/50'
                    }`}
                    title={m.label}
                  >
                    <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    ...
                    {sidebarOpen && <span className="flex-1 truncate">{m.label}</span>}
                  </NavLink>
                ))}
              </div>
            )}
