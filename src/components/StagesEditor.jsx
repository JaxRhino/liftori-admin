...
import { STAGE_COLORS, paletteTint, DEFAULT_STAGES } from '../lib/products'

/**
 * StagesEditor — manage the editable build stages (product_stages).
 * Add / rename / reorder / recolor / remove. Persists on save:
 *  - upserts every current row with sort_order = index order
 *  - deletes stages removed in the editor (and nulls any product override using them)
 */

let _n = 0
const localId = () => 'new_' + (++_n)
const slug = (s) => (s || '').toLowerCase().trim().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || 'stage'

export default function StagesEditor({ onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (!alive) return
        const src = data && data.length ? data : DEFAULT_STAGES
  ...
  async function save() {
    ...
    setSaving(true)
    try {
      // resolve keys (new rows get a slug from their label; keep existing keys stable)
      const used = new Set()
      const resolved = rows.map((r) => {
        let key = r.stage_key
        if (!key) {
          let base = slug(r.label); key = base; let n = 2
      ...
      const { error: upErr } = await supabase.from('product_stages').upsert(payload, { onConflict: 'stage_key' })
      if (upErr) throw upErr

      const finalKeys = new Set(resolved.map((r) => r.stage_key))
      const removed = initialKeys.filter((k) => !finalKeys.has(k))
      if (removed.length) {
        await supabase.from('product_workspaces').update({ stage: null }).in('stage', removed)
        await supabase.from('product_stages').delete().in('stage_key', removed)
      }
      onSaved && onSaved()
      onClose && onClose()
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4 sm:p-8" onClick={onClose}>
      <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-navy-900 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <div>
            <h2 className="text-lg font-bold text-white">Manage stages</h2>
            <p className="text-xs text-gray-400">Reorder, rename, recolor, add or remove the build stages used across all products.</p>
          </div>
