 ...
 * mobile reseller app (users, listings, community), its in-app Beta Testers
 * feedback program (tester_feedback / _messages), and the Founder Beta signup
 * list collected from liftori.ai/get-bolo (bolo_beta_signups).
 *
...
function exportBetaCsv(rows) {
  const head = ['Name', 'Email', 'State', 'Phone', 'Referral', 'Signed up', 'Emailed'];
  const lines = [head.join(',')].concat(
    (rows || []).map((r) => [
      r.full_name, r.email, r.state, r.phone_type, r.ref_code || '',
      r.created_at, r.invite_sent ? 'yes' : 'no',
    ].map(csvCell).join(','))
  );
  ...
  a.href = url;
  a.download = `bolo-beta-signups-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  async function load() {
    ...
    try {
      const [
        ...
        ordersRes, fbRes, msgCount, betaRes,
      ] = await Promise.all([
        ...
        // Founder Beta signups from liftori.ai/get-bolo
        supabase.from('bolo_beta_signups').select('full_name, email, state, phone_type, ref_code, invite_sent, created_at').order('created_at', { ascending: false }),
      ]);
      ...
      // Founder Beta signups
      const beta = betaRes.data || [];
      const betaAndroid = beta.filter((b) => b.phone_type === 'Android').length;
      const betaIphone = beta.filter((b) => b.phone_type === 'iPhone').length;
      const betaWeek = beta.filter((b) => b.created_at >= wk).length;
      const betaState = {};
      beta.forEach((b) => { if (b.state) betaState[b.state] = (betaState[b.state] || 0) + 1; });
      const betaTopStates = Object.entries(betaState).sort((a, b) => b[1] - a[1]).slice(0, 6);
      setM({
        ...
        betaTotal: beta.length, betaAndroid, betaIphone, betaWeek,
        betaTopStates, betaRecent: beta.slice(0, 6), betaAll: beta,
      });
    } catch (e) {
    ...
    <div>
      ...
      {loading || !m ? (
        ...
        <>
          ...
          {/* Founder Beta signups panel */}
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 mb-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <UserPlus className="w-4 h-4 text-sky-400" />
                <h3 className="text-sm font-bold text-white">Founder Beta Signups</h3>
                <span className="text-[11px] text-slate-500">liftori.ai/get-bolo</span>
              </div>
              <button onClick={() => exportBetaCsv(m.betaAll)} disabled={!m.betaTotal}
                className="flex items-center gap-1 text-xs text-sky-400 hover:text-sky-300 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                <Download className="w-3.5 h-3.5" /> Export CSV
              </button>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
              <Mini label="Total signups" value={m.betaTotal} tint="text-sky-400" />
              <Mini label="Android" value={m.betaAndroid} />
              <Mini label="iPhone (waitlist)" value={m.betaIphone} />
              <Mini label="This week" value={m.betaWeek} tint={m.betaWeek ? 'text-emerald-400' : 'text-white'} />
            </div>
            {m.betaTotal === 0 ? (
              <div className="text-center py-6 text-slate-500 text-sm">
                No signups yet - share liftori.ai/get-bolo in the Facebook post to start filling the Founder beta.
              </div>
            ) : (
              <>
                {m.betaTopStates.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    {m.betaTopStates.map(([st, v]) => (
                      <span key={st} className="text-[11px] px-2 py-0.5 rounded-full border border-slate-600/40 bg-white/5 text-slate-300">{st} · {v}</span>
                    ))}
                  </div>
                )}
                <div className="space-y-1.5">
                  {m.betaRecent.map((b, i) => {
                    const isPhone = b.phone_type === 'iPhone';
                    const badge = isPhone
                      ? 'text-slate-300 bg-white/5 border-slate-600/40'
                      : 'text-sky-400 bg-sky-500/10 border-sky-500/30';
                    return (
                      <div key={b.email + i} className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-white/5 transition-colors">
                        <span className="text-sm text-white truncate" style={{ flex: '0 0 30%' }}>{b.full_name}</span>
                        <span className="text-[12px] text-slate-400 truncate flex-1">{b.email}</span>
                        <span className="text-[11px] text-slate-500 hidden md:inline">{b.state}</span>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full border ${badge}`}>{b.phone_type}</span>
                        <span className="text-[11px] text-slate-500 w-8 text-right">{ago(b.created_at)}</span>
                      </div>
                    );
                  })}
                </div>
